...
3. **Read the CPU.** The AI loop at `0x08060B06` and its action dispatcher
   ...
   *Delivered for what the traces exercise* (DERIVATION 44, 45). The rig:
   ...
   abort. **The "nothing to do" fallback is read** (DERIVATION 49): a foot
   unit with no property left, or a mover with nothing reachable, falls
   into `0x0806606C`, which without a Port on the map is settle -- two
   traces requested by the harness's second abort. Not yet: movement
   modes 2, 3, 5, 6, 7, the Lander, the TCopter, the loaded transport's
   move, the retreat-after-move check, the fallback's Lander pickup and
   the foot unit's TCopter ride, firing a power, the TCopter and Lander
   purchases -- each raises
...
5. **Tuning against the port.** Planner against `engine/cpu_ai` in Python,
   ...
   *Begun: the harness.* `tools/sparring.py` plays the planner against the
   ...
   day 8, and the queue's next entry was `0x08064D6A`, a foot unit with
   no property left to walk to, from the P2 game's day 10 -- traced and
   ported in turn (DERIVATION 49). Playing P2
   ...
   loss two plies out, when no unit is left in range to stop it. That was
   a shape to fix before any weight is tuned, and it is fixed as a term,
   not a rule: `property_exposure` / `hq_exposure` price, on the board an
   action leaves, every own property an enemy foot unit can step onto
   next turn by the share it can hold there (the HQ at the win's value),
   and charge the action the change; the reply's evaluation carries the
   same exposure for both sides (`docs/ADVISOR.md`). Replayed with the
   term, the P1 game is the same rout on day 8 and the P2 game runs to
   day 13 with the port down to its last unit, where it aborts at
   `0x0801C120`, the CPU firing its power -- the trace queue's next entry
   now that `0x08064D6A` is ported. Both requests are regenerated by the same command
   (`tools/sparring.py tests/fixtures/cpu/vs15-p2-cpu.before.json
   --both-sides --aborts DIR`); the games are deterministic.
