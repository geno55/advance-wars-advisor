...
def main(rom_path, out_path):
    ...
    out = {
        "_comment": [
            ...
            "The three tables are weather variants, identified by the shape of",
            "their differences: table 1 raises 27 cells including foot units",
            "(snow), while table 2 differs in exactly 4 -- Treads and Tires on",
            "Plain and Wood, +1 each, foot untouched (rain turning soft ground",
            "to mud). Inferred from that signature, not read from a label.",
        ],
        "provenance": {"method": "extracted from ROM binary",
                       "offsets": [hex(b) for b in BASES],
                       "layout": "7 movement types x 20 terrain slots, u8",
                       "impassable": IMPASSABLE,
                       "weather_inferred_from_difference_pattern": True},
