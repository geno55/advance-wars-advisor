-- The 0x20 flag and the 24-byte table at 0x08283FC8 (DERIVATION 31).
--  A. bind: P2 unit turned Sub on written Sea, driven through its action
--     menu -- the write that lands on its flags byte names the Dive handler.
--  B. surface: same, starting from a written 0x20 -- the menu should offer
--     the counterpart and the write should clear the bit.
--  C. cruiser vs dived sub at contact: damage must come from the table (90).
--  D. bcopter vs the same dived sub: Fire must not be offered (table 0).
--  E. cruiser vs SURFACED sub: also 90 -- diving does not change the
--     hunters' number, it deletes everyone else's.
...
local log = io.open(OUT .. "dive.log", "w")
local function shot(tag)
  local png = emu.takeScreenshot()
  local fh = io.open(OUT .. "dv-" .. tag .. ".png", "wb")
...
local function main()
  ...
  -- A. dive binds the bit
  reload()
  w8(ua(71), 24)                     -- P2 Tank -> Sub
  w8(MAP + 7 * W + 9, SEA)           -- sea under it
  watch_addr = ua(71) + 1
  watching = true
  goto_tile(9, 7)
  tap("a", 6, 40)                    -- select
  ...
  wait(60)
  watching = false
  log:write(string.format("A: flags=%02X dived=%d\n",
  ...
  wait(60)
  watching = false
  log:write(string.format("B: flags=%02X dived=%d\n",
    r8(ua(71) + 1), math.floor(r8(ua(71) + 1) / 32) % 2))

  -- C. cruiser vs dived sub
  reload()
  w8(ua(2), 24)                      -- P1 unit 2 -> Sub
  w8(ua(2) + 1, 0x20)                -- dived
  w8(ua(2) + 2, 8); w8(ua(2) + 3, 7) -- teleport adjacent
  w8(MAP + 7 * W + 8, SEA)
  w8(ua(71), 22)                     -- P2 Tank -> Cruiser
  local hp0, chp0 = r16(ua(2) + 4) % 128, r16(ua(71) + 4) % 128
  goto_tile(9, 7)
  tap("a", 6, 40)
  tap("a", 6, 50)
  shot("C-menu")
  ...
  tap("a", 6, 30)                    -- confirm
  wait(500)
  local hp1, chp1 = r16(ua(2) + 4) % 128, r16(ua(71) + 4) % 128
  log:write(string.format("C: subhp %d->%d dmg=%d cruiser %d->%d counter=%d\n",
    hp0, hp1, hp0 - hp1, chp0, chp1, chp0 - chp1))

  -- D. bcopter vs the same dived sub: no Fire
  reload()
  w8(ua(2), 24); w8(ua(2) + 1, 0x20)
  w8(ua(2) + 2, 8); w8(ua(2) + 3, 7)
  w8(MAP + 7 * W + 8, SEA)
  w8(ua(71), 19)                     -- BCopter
  local bhp0 = r16(ua(2) + 4) % 128
  goto_tile(9, 7)
  tap("a", 6, 40)
  tap("a", 6, 50)
  shot("D-menu")
  tap("b", 6, 30); tap("b", 6, 30); tap("b", 6, 30)
  log:write(string.format("D: subhp still=%d\n", r16(ua(2) + 4) % 128))

  -- E. cruiser vs surfaced sub
  reload()
  w8(ua(2), 24)
  w8(ua(2) + 2, 8); w8(ua(2) + 3, 7)
  w8(MAP + 7 * W + 8, SEA)
  w8(ua(71), 22)
  local shp0 = r16(ua(2) + 4) % 128
  goto_tile(9, 7)
  tap("a", 6, 40)
  tap("a", 6, 50)
  tap("a", 6, 50)
  tap("a", 6, 30)
  wait(500)
  local shp1 = r16(ua(2) + 4) % 128
  log:write(string.format("E: subhp %d->%d dmg=%d\n", shp0, shp1, shp0 - shp1))
...
emu.addMemoryCallback(function()
  ...
  if coroutine.status(co) ~= "dead" then
    ...
    if not ok then
      local ef = io.open(OUT .. "dive_err.log", "w")
