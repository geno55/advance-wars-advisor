...
WHAT IS MODELLED
  ...
  * WAIT on any tile `destinations()` allows, with the exposure there.
...
WHAT IS NOT MODELLED, deliberately and visibly
  ...
  * CO powers. The charge is readable but the activation threshold is not
    known, so activating one is not an action here and nothing models a power
    firing mid-turn.
  ...
  * Terrain repair/resupply on WAIT. Real, unmeasured, absent.
...
class Action:
    ...
    `kind` is one of "attack", "capture", "load", "wait". The optional fields
...
def actions_for(board, unit, *, co_ids: Optional[dict] = None,
    ...
    for tile, cost in dests.items():
        ...
        out.append(Action(kind="wait", unit=unit, tile=tile, move_cost=cost,
                          exposure=ff, hp_after=unit.hp, **facts(tile)))
    ...
    for enemy in enemies:
        ...
        for tile in fire_from:
            ...
            out.append(Action(
                ...
                hp_after=counter.min_remaining_hp if counter else unit.hp))
